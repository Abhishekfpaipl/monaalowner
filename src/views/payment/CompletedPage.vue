<template>
    ...
    <div class="list-group list-flushed" style="padding-bottom: 100px;">
        <div v-for="(payment, index) in payments" :key="index" class="list-group-item  test">
            <div class="d-flex align-items-center" @click="showSupbill(payment)">
                ...
                <div class="flex-fill ms-2 fw-bold">
                    ...
                    <small class="flex-fill d-flex justify-content-between">
                        <p class="mb-0">{{ payment.bills }}</p>
                        <p v-if="payment.type === 'Supplier'" class="mb-0 text-danger">₹ {{ payment.amt }}</p>
                        <p v-if="payment.type === 'Buyer'" class="mb-0 text-success">₹ {{ payment.amt }}</p>
                    </small>
                </div>
            </div>
        </div>
...
export default {
    components: { PaymentLinks, BillOffcanvas },
    data() {
        return {
    ...
    methods: {
        showSupbill(payment) {
            return this.$store.dispatch('selectSupbill', payment);
        },
